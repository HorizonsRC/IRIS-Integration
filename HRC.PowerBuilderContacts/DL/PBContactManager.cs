...
namespace HRC.PowerBuilderContacts.DL
{
    internal class PBContactManager
    {
        ...
        internal static int Save(PBContact contact, bool newContact, string PBContactId="")
        {
            ...
            using (SqlConnection con = CommandHelper.CreateConnection(ConnInstance.PowerBuilderContacts))
            {
                using (SqlTransaction trans = con.BeginTransaction())
                {
                    ...
                    //save billing address (IsBilling=TRUE) else save postal address (IsPostal=TRUE)
                    ...
                    {//use postal no billing address found
                        //#BP 03/03/2016    After discussion with Sami, I'm removing the requirement for an address to be current if it carries the IsPostal flag (i.e. it's the preferred address)
                        //primaryAddress = contact.Addresses.Find(a => a.Base.IsPostal.Equals(true) && a.Base.IsCurrent.Equals(true));
                        primaryAddress = contact.Addresses.Find(a => a.Base.IsPostal.Equals(true));
                    }
                    if (primaryAddress != null)
                    {
                    ...
                    //#BP 04/12/2015    Prevent creation of multiple primary flags per contact
                    //#BP Work out a best primary comm point in case there are multiple (use a phone number if possible)
                    PBCommunication bestPrimary = null;
                    foreach (PBCommunication communication in contact.Communications)
                    {
                        if (communication.Base.BasicCommunicationType == "phone" && (bool)communication.Base.PrimaryFlag)
                        {//#BP this gets the primary phone number (most cases will have this)
                            bestPrimary = communication;
                        }
                    }

                    foreach (PBCommunication communication in contact.Communications)
                    {//step through all the comm points to update the database
                        if (bestPrimary != null)
                        {//#BP  check if we've identified a best primary phone number above...
                            if (!communication.Equals(bestPrimary))
                            {//#BP if we have, but this is not it then set the primary flag to false, ensuring only one primary flag is set
                                communication.Base.PrimaryFlag = false;
                            }
                        }
                        communication.Base.ContactId = Id;
        ...
        {            
            ...
            {                
                using (SqlCommand command = con.CreateCommand())
                {
                    ...
                    //BP 04/12/2015 Set Id = NextId so the following unique PrimaryFlag enforcer can avoid this ID
                    Id = nextId;
                }
            }
            //#BP 04/12/2015    Added this section to ensure only one primary flag per agent_id, so if the newly added or updated record has the primary flag set
            //                  then set all other flags for the nominated agent_id to 'N'
            if ((bool)communication.Base.PrimaryFlag)
            {
                using (SqlCommand cmd = con.CreateCommand())
                {
                    cmd.Transaction = trans;
                    cmd.CommandText = @"UPDATE [CONT_COMM_POINT] SET primary_flag = 'N' WHERE agent_id = " + communication.Base.ContactId.ToString() + " AND cp_id <> " + Id.ToString();
                    cmd.ExecuteNonQuery();
                }
            }
            //#BP 04/12/2015    End of change
        }
                        ...
                        new Tuple<string, object>("AGENT_ID", PBContactId)))
                {
                    ...
                    using (SqlCommand command = con.CreateCommand())
                    {
                        ...
                        command.CommandText = string.Format(sql,
                            ...
                            command.CreateParameter("FirstName", $"{person.Base.FirstName} {person.Base.MiddleName}"),
        ...
        {            
            //Save the address to PowerBuilder cont_address
            int Id = 0;
            //If it's an update, verify that the address record already exists for this contact before proceeding
            if (!newContact && IdentityExists(con, trans, "CONT_ADDRESS", "ADDRESS_ID", ref Id, new Tuple<string, object>("AGENT_ID", PBContactId)))
            {
                //All good, so update
                using (SqlCommand command = con.CreateCommand())
                {
                    ...
                    //Populate the SQL update command with the various address attributes
                    command.CommandText = string.Format(sql,
                        command.CreateParameter("Id", Id),
            ...
            {
                //Address does not already exist, so insert
