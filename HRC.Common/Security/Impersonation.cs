...
namespace HRC.Common.Security
{
    public class Impersonation : IDisposable
    {
        ...
        private IntPtr m_TokenDuplicate;
        private IntPtr m_Token;

        public Impersonation(NetworkCredential credential)            
        {
        ...
        private void Logon()
        {
            if (!LogonUser(this.Credential.UserName, this.Credential.Domain, this.Credential.Password, LogonType.Interactive, LogonProvider.Default, out m_Token))
            {
                throw new Win32Exception();
            }
            if (!DuplicateToken(m_Token, SecurityImpersonationLevel.Impersonation, out m_TokenDuplicate))
            {
                throw new Win32Exception();
            }
            ImpersonationContext = new WindowsIdentity(m_TokenDuplicate).Impersonate();
        }
        ...
        public void Dispose()
        {
            ...
            if (m_TokenDuplicate != IntPtr.Zero)
            {
                if (!CloseHandle(m_TokenDuplicate))
                {
                    throw new Win32Exception();
                }
            }
        ...
        public static void RunAction(string userName, string password, string domain, Action action)
        {
            IntPtr token;

            if (!LogonUser(userName, domain, password, LogonType.NewCredentials, LogonProvider.Default, out token))
            {
                throw new Win32Exception();
            }
            ...
            try
            {
                ...
                if (!DuplicateToken(token, SecurityImpersonationLevel.Impersonation, out tokenDuplicate))
                {
                    throw new Win32Exception();
                }
